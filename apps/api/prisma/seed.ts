...
async function main() {
  ...
  const existingRestaurantCard = await prisma.card.findFirst({ where: { userId: restaurant.id } });
  if (!existingRestaurantCard) {
    await prisma.card.create({
      data: {
  ...
  const existingAgencyCard = await prisma.card.findFirst({ where: { userId: agency.id } });
  if (!existingAgencyCard) {
    await prisma.card.create({
      data: {
        userId: agency.id,
        username: 'luminous',
  ...
  const restaurantCard = await prisma.card.findFirst({ where: { userId: restaurant.id } });
  const agencyCard = await prisma.card.findFirst({ where: { userId: agency.id } });
