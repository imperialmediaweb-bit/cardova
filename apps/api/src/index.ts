...
async function seedDemoAccounts() {
  try {
    ...
    const existingAdminCard = await prisma.card.findFirst({ where: { userId: admin.id } });
    if (!existingAdminCard) {
      await prisma.card.create({
        data: { userId: admin.id, username: 'admin', displayName: 'Cardova Admin', title: 'Platform Administrator', company: 'Cardova', location: 'San Francisco, CA', bio: 'Building the future of digital business cards.', theme: 'glass', isPublished: true, socialLinks: { website: 'https://cardova.app', email: 'admin@cardova.app' } },
      });
    }
    ...
    const existingBellaCard = await prisma.card.findFirst({ where: { userId: bella.id } });
    if (!existingBellaCard) {
      await prisma.card.create({
        data: {
    ...
    const existingLuminousCard = await prisma.card.findFirst({ where: { userId: luminous.id } });
    if (!existingLuminousCard) {
      await prisma.card.create({
        data: {
